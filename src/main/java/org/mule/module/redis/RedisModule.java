...
@Module(name = "redis", schemaVersion = "3.7", friendlyName = "Redis", minMuleVersion = "3.7.0", description = "Redis Module")
public class RedisModule implements PartitionableObjectStore<Serializable>, MuleContextAware {
    ...
    private JedisStrategy jedisStrategy;
    ...
    public void initializeJedis() {
        ...
        if (StringUtils.isNotBlank(clusterHosts)) {
            ...
            jedisClusterNodes.addAll(parseHosts(clusterHosts));
            JedisCluster jedisCluster = new JedisCluster(jedisClusterNodes, poolConfig);
            jedisStrategy = new ClusteredJedisStrategy(jedisCluster);
        } else {
            JedisPool jedisPool = new JedisPool(poolConfig, host, port, connectionTimeout, password);
            jedisStrategy = new PooledJedisStrategy(jedisPool);
    ...
    public byte[] set(final String key,
        ...
        return RedisUtils.run(jedisStrategy, new RedisAction<byte[]>() {
            @Override
            public byte[] run() {
    ...
    public Boolean exists(final String key) {
        return RedisUtils.run(jedisStrategy, new RedisAction<Boolean>() {
            @Override
            public Boolean run() {
                final byte[] keyAsBytes = SafeEncoder.encode(key);
    ...
    public Long increment(final String key, @Optional @Default("1") final long step) {
        return RedisUtils.run(jedisStrategy, new RedisAction<Long>() {
            @Override
            public Long run() {
                final byte[] keyAsBytes = SafeEncoder.encode(key);
    ...
    public static enum ListPushSide {
        LEFT {
            ...
            @Override
            byte[] push(final JedisStrategy redis,
                        final byte[] key,
                        final byte[] message,
            ...
            @Override
            byte[] pop(final JedisStrategy redis, final byte[] key) {
        ...
        abstract byte[] push(JedisStrategy redis, byte[] key, byte[] message, boolean ifNotExists);

        abstract byte[] pop(JedisStrategy redis, final byte[] key);
    }
    ...
    public byte[] pushToList(final String key,
        ...
        return RedisUtils.run(jedisStrategy, new RedisAction<byte[]>() {
            ...
            public byte[] run() {
                final byte[] valueAsBytes = RedisUtils.toBytes(value, muleEvent.getEncoding());
                return side.push(jedisStrategy, SafeEncoder.encode(key), valueAsBytes, ifExists);
            }
        });
    }
    ...
    public static enum SortedSetOrder {
        ASCENDING {
            @Override
            Set<byte[]> getRangeByIndex(final JedisStrategy redis,
                                        final byte[] key,
                                        final int start,
                                        final int end) {
            ...
            @Override
            Set<byte[]> getRangeByScore(final JedisStrategy redis,
                                        final byte[] key,
                                        final double min,
                                        final double max) {
        ...
        abstract Set<byte[]> getRangeByIndex(JedisStrategy redis, final byte[] key, int start, int end);

        abstract Set<byte[]> getRangeByScore(JedisStrategy redis, final byte[] key, double min, double max);
    }
    ...
    public Set<byte[]> getRangeByIndex(final String key,
        ...
        return RedisUtils.run(jedisStrategy, new RedisAction<Set<byte[]>>() {
            @Override
            public Set<byte[]> run() {
                final byte[] keyAsBytes = SafeEncoder.encode(key);
    ...
    public Double incrementSortedSet(final String key,
                                     ...
                                     @Optional @Default("#[message.payloadAs(java.lang.String)]") final String value,
                                     final MuleEvent muleEvent) {
        return RedisUtils.run(jedisStrategy, new RedisAction<Double>() {
            @Override
            public Double run() {
                final byte[] keyAsBytes = SafeEncoder.encode(key);
                final byte[] valueAsBytes = RedisUtils.toBytes(value, muleEvent.getEncoding());
    ...
    public void subscribe(final List<String> channels, final SourceCallback callback) {
        while (running) {
            try {
                RedisUtils.run(jedisStrategy, new RedisAction<Void>() {
                    @Override
                    public Void run() {
    ...
    public void store(final Serializable key, final Serializable value, final String partitionName)
        ...
        final Long result = RedisUtils.run(jedisStrategy, new RedisAction<Long>() {
            @Override
            public Long run() {
    ...
    public Serializable retrieve(final Serializable key, final String partitionName)
            throws ObjectStoreException {
        final Serializable result = RedisUtils.run(jedisStrategy, new RedisAction<Serializable>() {
            @Override
    ...
    public List<Serializable> allKeys(final String partitionName) throws ObjectStoreException {
        return RedisUtils.run(jedisStrategy, new RedisAction<List<Serializable>>() {
            @Override
    ...
    public List<String> allPartitions() throws ObjectStoreException {
        return RedisUtils.run(jedisStrategy, new RedisAction<List<String>>() {
            @Override
    ...
    public void disposePartition(final String partitionName) throws ObjectStoreException {
        RedisUtils.run(jedisStrategy, new RedisAction<Long>() {
            @Override
            public Long run() {
    ...
    public List<HostAndPort> parseHosts(String hosts) {
